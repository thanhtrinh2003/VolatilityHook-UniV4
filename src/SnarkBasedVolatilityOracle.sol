...
contract SnarkBasedVolatilityOracle is RvVerifier, Ownable {
    ...
    function verifyAndUpdate(uint256 claimed_s, bytes memory proof, bytes memory publicValues) public {
        // Verify the proof using the base contract's function
        (bytes8 n_inv_sqrt, bytes8 n1_inv, bytes8 new_s2, bytes8 n_bytes, bytes32 digest) =
            this.verifyRvProof(proof, publicValues);
    ...
    function n_sqrt_test(bytes8 n_inv_sqrt, bytes8 n_bytes) public pure returns (bool) {
        ...
        uint256 error = 2 * n_inv_sqrt_256 + 1;
    ...
    function n1_check(bytes8 n_bytes, bytes8 n1_inv) public pure returns (bool) {
        ...
        uint256 error = 2 * n1_inv_256 + 1;
        uint256 n1_inv_test = n1_inv_256 * n1 >> fraction_bits;
        uint256 one_256 = 1 << fraction_bits;
    ...
    function s2_check(uint256 claimed_s, bytes8 new_s2) public pure returns (bool) {
        uint256 s_test = uint256(uint64(claimed_s));
        uint256 s2 = uint256(uint64(new_s2));
        uint256 s2_test = s_test * s_test >> fraction_bits;
        uint256 error = 2 * s_test + 1;
