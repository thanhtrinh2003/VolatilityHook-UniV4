...
contract Quoter is IQuoter, IUnlockCallback {
    ...
    {
        try manager.unlock(abi.encodeWithSelector(this.quoteExactInputSingleSelfCall.selector, params)) {}
    ...
    {
        try manager.unlock(abi.encodeWithSelector(this.quoteExactInputSelfCall.selector, params)) {}
        catch (bytes memory reason) {
    ...
    function quoteExactOutputSingle(QuoteExactSingleParams memory params)
        ...
        returns (int128[] memory deltaAmounts, uint160 sqrtPriceX96After, uint32 initializedTicksLoaded)
    {
        try manager.unlock(abi.encodeWithSelector(this.quoteExactOutputSingleSelfCall.selector, params)) {}
        catch (bytes memory reason) {
    ...
    {
        try manager.unlock(abi.encodeWithSelector(this.quoteExactOutputSelfCall.selector, params)) {}
        catch (bytes memory reason) {
            return _handleRevert(reason);
        }
    }
    ...
    function quoteExactInputSelfCall(QuoteExactParams memory params) public selfOnly returns (bytes memory) {
    ...
    function quoteExactOutputSelfCall(QuoteExactParams memory params) public selfOnly returns (bytes memory) {
        uint256 pathLength = params.path.length;
